...
# Second time 27 Jan 2021 -- easy to understand approach
class Solution2(object):
    def PredictTheWinner(self, nums):
        ...
        """
        d = [[-1]*len(nums) for i in range(len(nums))]

        def relativeScore(i, j):
            ...
            s1 = nums[i]-relativeScore(i+1, j)
            s2 = nums[j]-relativeScore(i, j-1)
